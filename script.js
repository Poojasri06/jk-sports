...
function renderCatalogItem(item, categoryKey, itemIndex) {
  ...
  var itemId = (item && item.id) ? item.id : (categoryKey + '-' + itemIndex);
  var hasPrice = item && typeof item.price === 'number';
  var inStock = !!(item && item.stock);

  var priceHtml = hasPrice
    ? '<p class="catalog-item-price">' + formatINR(item.price) +
      (item.mrp && item.mrp > item.price ? ' <span class="catalog-item-mrp">' + formatINR(item.mrp) + '</span>' : '') +
      '</p>'
    : '';

  var addToCartData = JSON.stringify({ id: itemId, name: item.name, price: item.price || 0, img: imgSrc }).replace(/"/g, '&quot;');
  var addToCartHtml = (hasPrice && inStock)
    ? '<button type="button" class="btn-add-cart" onclick=\'addToCart(' + addToCartData + ')\'>Add to Cart</button>'
    : '';
...
function openCatalog(key) {
  ...
  document.body.setAttribute('data-open-catalog', key);
}
...
function closeModal() {
  ...
  document.body.removeAttribute('data-open-catalog');
}
